...
namespace NotesPlus
{
	...
	public class DoYourThing
	{
		...
		{
			string setting = ModSettings.GetString("Additional Notes Style");
			string prefix = (setting == "(Note)") ? "(" : ((setting == "[Note]") ? "[" : ((setting == "{Note}") ? "{" : ((setting == "- Note") ? "- " : "")));
			string suffix = (prefix == "(") ? ")" : ((prefix == "[") ? "]" : ((prefix == "{") ? "}" : ""));
